...
import CryptoHelp from "../CryptoHelp/CryptoHelp";
...
const Crypto = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await axios.post("/api/provider/Crypto", form);

      if (res.data.success) {
        toast.success("Crypto submitted successfully!");
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="max-w-2xl bg-white/90 backdrop-blur-md shadow-2xl rounded-2xl p-8 border border-blue-100">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Crypto Type</label>
                <select
                  name="cardType"
              ...
              >
                {loading ? "Submitting..." : "Submit Crypto"}
              </button>
            </form>
          </div>
        </div>

        <CryptoHelp data={form} />
      </div>
    </div>
  );
};
